...
async function forceMigrateToFirebase(key: string, songs: Cancion[]) {
  ...
  try {
    ...
    console.log('✅ Migración forzada exitosa:', songs.length, 'canciones')
    return true
  } catch (error) {
...
type Cancion = {
  ...
  estado: Estado
  estilo?: string
  genero?: string  // 'Cantante masculino' | 'Cantante femenina' - solo para libres
  prioridad?: string  // 'A' | 'B' | 'C' para Alta, Media, Baja
...
function useFirebaseStore(key: string, initial: Cancion[]) {
  const [data, setData] = useState<Cancion[]>(() => {
    // Cargar desde localStorage primero mientras se carga Firebase
    try {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Intentar leer los datos iniciales desde Firebase
      getDoc(docRef).then((docSnap) => {
        if (docSnap.exists()) {
          const firebaseData = docSnap.data().songs || initial
          console.log('📥 Cargado desde Firebase:', firebaseData.length, 'canciones')
          // Comparar con localStorage - preferir localStorage si tiene más datos (más reciente)
          try {
            const raw = localStorage.getItem(key)
            if (raw) {
              const localData = JSON.parse(raw)
              if (localData.length >= firebaseData.length) {
                console.log('✅ localStorage tiene datos más recientes, usando localStorage')
                setData(localData)
                // Sincronizar localStorage a Firebase
                ignoreNextSnapshot.current = true
                setDoc(docRef, { songs: localData }).then(() => {
                  setTimeout(() => { ignoreNextSnapshot.current = false }, 1000)
                }).catch(console.error)
              } else {
          ...
          // Si no existe en Firebase, usar localStorage
          try {
            const raw = localStorage.getItem(key)
            if (raw) {
              const localData = JSON.parse(raw)
              console.log('📥 Cargado desde localStorage:', localData.length, 'canciones')
              setData(localData)
              // Subir a Firebase
              ignoreNextSnapshot.current = true
              setDoc(docRef, { songs: localData }).then(() => {
              ...
              console.log('📭 No hay datos, iniciando vacío')
              setData(initial)
            }
          } catch {
            setData(initial)
        ...
        try {
          ...
          if (raw) {
            const localData = JSON.parse(raw)
            console.log('📥 Cargado desde localStorage (fallback):', localData.length, 'canciones')
            setData(localData)
          }
        } catch {}
        setIsLoading(false)
      ...
      // Escuchar cambios en tiempo real (solo de otros dispositivos o cambios externos)
      const unsubscribe = onSnapshot(docRef, (docSnap) => {
        // Ignorar si acabamos de guardar nosotros mismos (evita sobrescribir nuestros propios cambios)
        if (ignoreNextSnapshot.current) {
          // Resetear después de un delay para permitir sincronización de otros dispositivos
          setTimeout(() => {
        ...
        if (docSnap.exists()) {
          // Solo aplicar cambios si no tenemos escrituras pendientes (evita conflictos)
          if (!docSnap.metadata.hasPendingWrites) {
            const newData = docSnap.data().songs || initial
            setData(newData)
      ...
      console.error('Error inicializando Firebase store:', error)
      setIsLoading(false)
    }
  }, [key, initial])

  // Función para actualizar que escribe a Firebase y localStorage
  const updateData = (newData: Cancion[] | ((prev: Cancion[]) => Cancion[])) => {
    setData(prevData => {
      const updated = typeof newData === 'function' ? newData(prevData) : newData
      
      // Siempre guardar en localStorage PRIMERO (esto es lo más importante)
      try {
        localStorage.setItem(key, JSON.stringify(updated))
        console.log('✅ Guardado en localStorage:', updated.length, 'canciones')
      } catch (err) {
      ...
      if (db) {
        try {
          ...
          ignoreNextSnapshot.current = true
          setDoc(docRef, { songs: updated }).then(() => {
            console.log('✅ Guardado en Firebase exitoso')
            // Dar tiempo para que se procese antes de permitir snapshots
            setTimeout(() => {
              ignoreNextSnapshot.current = false
            }, 1000)
          }).catch((err) => {
...
// Mantener useLocalStore para preferencias (no necesitan sincronización)
...
export default function App({ onLogout }: { onLogout?: () => Promise<void> }){
  ...
  const [sortBy, setSortBy] = useLocalPreference<'nombre'|'genero'|'estilo'|'prioridad'>('catalogo-sort', 'nombre')
  ...
  async function migrateToFirebase(){
    ...
    if(songs.length === 0){
      setToast('No hay canciones para migrar')
      setTimeout(()=>setToast(null), 3000)
      return
    }
    
    try {
      const docRef = doc(db, 'catalogo', 'catalogo-bks-v4')
      await setDoc(docRef, { songs })
      setToast(`✅ ${songs.length} canción(es) migradas a Firebase exitosamente`)
      setTimeout(()=>setToast(null), 4000)
    } catch (error) {
      console.error('Error migrando a Firebase:', error)
      setToast('❌ Error al migrar a Firebase. Revisa la consola.')
      setTimeout(()=>setToast(null), 4000)
    }
  }
      ...
      try {
        ...
        'Link MP3 máster': 'linkMP3Master'
          ...
          for(let i = 1; i < jsonData.length; i++){
            ...
            // Primero verificar si la fila tiene datos reales antes de procesar
            const hasRealData = row.some((cell: any, idx: number) => {
              const header = headers[idx]
              const value = cell
              // Verificar que hay un valor real en alguna celda
            ...
            // Verificar si la canción tiene al menos un campo con datos reales después de procesar
            const hasValidData = Object.keys(song).length > 1 && (song.titulo || song.estilo || song.artista || song.editorial)
            
            if(hasValidData) {
              // Solo asignar valores por defecto si hay datos
  ...
  function exportToExcel(){
    ...
    const headersLibres = [
      'Título', 'Prioridad', 'Estado', 'Estilo', 'Género', 'Cliente objetivo', 'Fecha de creación', 'Tempo/Key', 
    ...
    // Función para limpiar saltos de línea y convertir a una sola línea
    ...
    function createLibresSheet(){
      ...
      // Título más ancho, Letra y Notas más estrechas para ocultar texto largo
      const wscols = headersLibres.map((_, idx) => {
        if(idx === 0) return { wch: 30 }  // Título
        if(idx === 1) return { wch: 12 }  // Prioridad
        if(idx === 9 || idx === 17) return { wch: 8 }  // Letra, Notas
        return { wch: 15 }  // Resto
      })
      ws['!cols'] = wscols
      
      // Establecer altura fija de filas para evitar expansión (header 20px, datos 15px)
      const range = XLSX.utils.decode_range(ws['!ref'] || 'A1')
      ws['!rows'] = [{ hpx: 20 }, ...Array(range.e.r).fill({ hpx: 15 })]
    ...
    function createColocadasSheet(){
      ...
      // Título más ancho, Letra y Notas más estrechas para ocultar texto largo
      const lastIdx = headersColocadas.length - 1  // Índice de Notas
      const wscols = headersColocadas.map((_, idx) => {
        if(idx === 0) return { wch: 30 }  // Título
        if(idx === 8 || idx === lastIdx) return { wch: 8 }  // Letra, Notas
        return { wch: 15 }  // Resto
      })
      ws['!cols'] = wscols
    ...
    if(libres.length > 0) {
      ...
      const wscols = headersLibres.map((_, idx) => {
        if(idx === 0) return { wch: 30 }  // Título
        if(idx === 8 || idx === 16) return { wch: 8 }  // Letra, Notas
        return { wch: 15 }  // Resto
      })
      wsLibresEmpty['!cols'] = wscols
      wsLibresEmpty['!rows'] = [{ hpx: 20 }]  // Altura fija
    ...
    if(colocadas.length > 0) {
      ...
      const lastIdxEmpty = headersColocadas.length - 1
      const wscols = headersColocadas.map((_, idx) => {
        if(idx === 0) return { wch: 30 }  // Título
        if(idx === 8 || idx === lastIdxEmpty) return { wch: 8 }  // Letra, Notas
        return { wch: 15 }  // Resto
      })
      wsColocadasEmpty['!cols'] = wscols
      wsColocadasEmpty['!rows'] = [{ hpx: 20 }]  // Altura fija
    ...
    <div>
      ...
      <div className="container">
        ...
        <div style={{display:'flex', justifyContent:'flex-end', alignItems:'center', gap:8, marginBottom:8}}>
          ...
          <select className="input" value={sortBy} onChange={e=>setSortBy(e.target.value as 'nombre'|'genero'|'estilo'|'prioridad')} style={{width:'auto', maxWidth:200}}>
            <option value="nombre">Nombre (A-Z)</option>
            <option value="genero">Género (M/F/M+F)</option>
...
function PriorityBadge({ prioridad }: { prioridad?: string }){
  ...
  const config = {
    'A': { label: 'Alta', color: '#ef4444', bg: 'rgba(239, 68, 68, 0.15)' },
    'A1': { label: 'Media Alta', color: '#f97316', bg: 'rgba(249, 115, 22, 0.15)' },
    'B': { label: 'Media', color: '#f59e0b', bg: 'rgba(245, 158, 11, 0.15)' },
    'C': { label: 'Baja', color: '#94a3b8', bg: 'rgba(148, 163, 184, 0.15)' }
  }
  
  const style = config[prioridad as keyof typeof config] || config['B']
...
function InlineEditor({ value, onChange, options }: { value?: string; onChange: (newValue: string) => void; options?: string[] }){
...
function GenreSelector({ genero, onChange }: { genero?: string; onChange: (newGenero: string) => void }){
          ...
          {options.map(option => (
            <div
              ...
              style={{
                padding: '6px 10px',
                cursor: 'pointer',
                background: option === genero ? 'rgba(0,0,0,.05)' : 'transparent',
                borderLeft: option === genero ? '3px solid #000' : '3px solid transparent',
                transition: 'all 0.2s',
                fontSize: '11px',
                fontWeight: '500'
              }}
              onMouseEnter={e => {
                if(option !== genero) e.currentTarget.style.background = 'rgba(0,0,0,.03)'
              }}
              onMouseLeave={e => {
                if(option !== genero) e.currentTarget.style.background = 'transparent'
              }}
            >
              {option}
            </div>
...
function PrioritySelector({ prioridad, onChange }: { prioridad?: string; onChange: (newPrioridad: string | undefined) => void }){
  const [isOpen, setIsOpen] = useState(false)
  const ref = useRef<HTMLDivElement>(null)
  ...
  const current = prioridad ? config[prioridad as keyof typeof config] : null
    ...
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <div 
        ...
        style={{
          ...
          alignItems: 'center',
          justifyContent: 'center',
          padding: prioridad ? '2px 6px' : '2px 8px',
          borderRadius: '4px',
          fontSize: '10px',
          fontWeight: '600',
          background: current?.bg || 'rgba(0,0,0,.05)',
          color: current?.color || '#666',
          border: prioridad ? `1px solid ${current?.color}40` : '1px dashed rgba(0,0,0,.2)',
          cursor: 'pointer',
          transition: 'all 0.2s',
          userSelect: 'none',
          minWidth: prioridad ? '60px' : '35px'
        }}
      >
        {prioridad ? current?.label : '•'}
      </div>
          ...
          {(['A', 'A1', 'B', 'C'] as const).map(key => (
            <div
              ...
              style={{
                padding: '6px 10px',
                cursor: 'pointer',
                background: key === prioridad ? config[key].bg : 'transparent',
                borderLeft: key === prioridad ? `3px solid ${config[key].color}` : '3px solid transparent',
                transition: 'all 0.2s',
                fontSize: '11px',
                fontWeight: '500',
              ...
              onMouseEnter={e => {
                if(key !== prioridad) e.currentTarget.style.background = 'rgba(0,0,0,.03)'
              }}
              onMouseLeave={e => {
                if(key !== prioridad) e.currentTarget.style.background = 'transparent'
              }}
            >
              {config[key].label}
            </div>
  ...
  return (
    <div className="card">
      <table>
        <thead>
          <tr>
            {isLibres && <th style={{width: '85px', padding: '8px'}}>Prioridad</th>}
            <th>Título</th>
            <th>Estilo</th>
            {isLibres && <th>Género</th>}
            <th>Reproducir</th>
          </tr>
        </thead>
        <tbody>
          ...
          {data.length === 0 && <tr><td colSpan={isLibres ? 5 : 3} className="muted">No hay canciones.</td></tr>}
        </tbody>
      </table>
    </div>
  )
  ...
  return (
    <div style={{
      ...
      <button onClick={togglePlay} style={{
        ...
        fontSize:12,
        lineHeight:1
      }} onMouseEnter={e=>{e.currentTarget.style.color='#000'; e.currentTarget.style.transform='scale(1.1)'}} onMouseLeave={e=>{e.currentTarget.style.color='#666'; e.currentTarget.style.transform='scale(1)'}}>
        {playing ? '⏸' : '▶'}
      </button>
      <input
        type="range"
        ...
        style={{
          flex:1,
          height:2,
          background:`linear-gradient(to right, #000 0%, #000 ${progress}%, #e0e0e0 ${progress}%, #e0e0e0 100%)`,
          borderRadius:1,
          outline:'none',
          cursor:'pointer',
      ...
      />
      <span style={{color:'#999', fontSize:11, minWidth:65, textAlign:'right', flexShrink:0}}>
...
function LetraField({value, onChange, placeholder}:{value?:string; onChange:(v:string)=>void; placeholder?:string}){
  ...
  // Detectar si es URL
  const isUrl = value && (value.startsWith('http://') || value.startsWith('https://') || value.includes('dropbox.com') || value.includes('.txt') || value.includes('www.'))
  
  // Si es URL, mostrar solo iconos
  if(isUrl) {
    return (
      <>
        <div style={{display:'flex', gap:12, alignItems:'center'}}>
          <a 
            ...
            title="Abrir URL de la letra"
          >
            <svg width="16" height="10" viewBox="0 0 22 14" fill="none">
              <ellipse cx="11" cy="7" rx="10" ry="6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" fill="none"/>
              <circle cx="11" cy="7" r="3.5" stroke="currentColor" strokeWidth="1.5" fill="none"/>
        ...
        {open && (<div className="overlay" onClick={()=>setOpen(false)}>
          <div className="overlay-card" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{marginBottom:8, fontSize:12, color:'var(--sub)'}}>
              Ingresa la URL del archivo con la letra:
            </div>
            <input 
              type="text" 
              value={value||''} 
  ...
  // Si no es URL, mostrar el campo normal
  return (
    <div style={{position:'relative'}}>
      <textarea 
      ...
      {open && (<div className="overlay" onClick={()=>setOpen(false)}>
        <div className="overlay-card" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{marginBottom:8, fontSize:12, color:'var(--sub)'}}>
            {isUrl ? 'Ingresa la URL del archivo con la letra:' : 'Escribe la letra aquí'}
          </div>
          {isUrl ? (
            <input 
              type="text" 
