...
export default function App(){
  ...
  const [sortBy, setSortBy] = useLocalPreference<'nombre'|'genero'|'estilo'>('catalogo-sort', 'nombre')
  ...
  function exportToExcel(){
    ...
    const headersLibres = [
      'Título', 'Estado', 'Estilo', 'Género', 'Cliente objetivo', 'Fecha de creación', 'Tempo/Key', 
    ...
    <div>
      ...
      <div className="container">
        ...
        <div style={{display:'flex', justifyContent:'flex-end', alignItems:'center', gap:8, marginBottom:8}}>
          ...
          <select className="input" value={sortBy} onChange={e=>setSortBy(e.target.value as 'nombre'|'genero'|'estilo')} style={{width:'auto', maxWidth:200}}>
          ...
          {data.map(c => {
            ...
            return (
              <tr key={c.id}>
                ...
                <td>{c.estilo || '—'}</td>
                {isLibres && <td>{c.genero || '—'}</td>}
                <td className="audio">{src ? <AudioPlayer key={c.id} id={c.id} src={src} /> : <span className="muted">—</span>}</td>
              </tr>
            )
          })}
          {data.length === 0 && <tr><td colSpan={isLibres ? 4 : 3} className="muted">No hay canciones.</td></tr>}
        </tbody>
      </table>
    </div>
  )
